...
import pdb
import pylab as plt
sys.path.insert(0,'/home/yuzhile/work2017/')
from toolboxes.visual.draw_bbox import draw_bbox
import skimage.draw

def circle(img, pts, color, radius):
    # Draw a circle
        ...
        for fimage in dataset:
            ...
            try:
                ...
                if args.show == 'true':
                    show_img = img.copy()
                    pdb.set_trace()
                    #draw_bbox(show_img,bounding_boxes[:,:-1],['box'],form='diagonal')
                    draw_bbox(show_img,[_bbox],['box'],form='diagonal')
                    show_img = circle(show_img,_landmark,[0,155,0],2)
                    plt.subplot(2,1,1)
                    plt.imshow(show_img)
                    plt.subplot(2,1,2)
                    plt.imshow(warped)
...
def parse_arguments(argv):
    ...
    parser.add_argument('--show', type=str, help='Show the detection result.', default='false')
